...
<template>
  <footer class="bg-light-gray">
      ...
      <ul class="site-link">
        <li v-for="item in siteMap" :key="item.title">
          ...
          <ul v-if="item.sublist">
            ...
            <router-link :to="`{path: '${item.routerLink}', hash: '${subItem.hash}'}`" class="dark sub-link">{{ subItem.linkName }}</router-link>
            </li>
          </ul>
        </li>
      </ul>
      ...
      <ul class="btn-area">
        <li>
          <RouterLink :to="reserveBtn.path" id="reserve-btn-footer">
            <img :src="getImageUrl('headerFooter/reservation-footer.png')"
            :alt="reserveBtn"/>
          <p class="dark">{{ reserveBtn.name }} </p></RouterLink>
        </li>
        <li>
          <RouterLink :to="shopBtn.path" id="shop-btn-footer"
          ><img :src="getImageUrl('headerFooter/shop-footer.png')" :alt="shopBtn" />
    ...
    <div class="bg-blue-3 container">
      <div class="subscribe">
        <p class="white01">訂閱 NORA Camp <br class="br-mobile">電子報，<br >與我們一同分享露營<br class="br-mobile">和貓狗的美好時光！</p>
        <div class="box">
      ...
      <div class="information">
        <div class="logo"><img :src="getImageUrl('headerFooter/logo-mobile-light.png')" alt="logo-image"></div>
    ...
    <div class="tinyp">本網站為緯育TibaMe_前端設計工程師班第88期學員專題成果作品，本平台僅供學習、展示之用。若有抵觸有關著作權，或有第三人主張侵害智慧財產權等情事，均由學員負法律上責任，緯育公司概不負責。若有侵權疑慮，您可以私訊［緯育TibaMe］，後續會由專人協助處理。</div>
  </footer>
</template>
